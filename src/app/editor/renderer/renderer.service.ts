...
import {TreeLayout} from "../shared";
import { LoggerService, RENDERER_DEFAULTS } from "../../shared";
...
export class Renderer {
  ...
  updateSimulation(ets) {
    let simDone = this.simulationMask.selectAll("rect.maskrect");
    simDone.forEach(
      aMask => {
        let taskId = aMask.data("fortask");
  ...
  selectTask(taskId) {
    let curTask = this.getTaskNodeById(taskId);
  ...
  private renderTaskTree(root: Task) {
    ...
    (function traverseBF(currentNode: Task) {
      let aTaskGroup = _this.svgCanvas.g().attr({
        class: "task",
        id: currentNode.id,
      });
  ...
  private renderTaskNode(cx, cy, origNode: Task, group) {
    ...
    let node = this.svgCanvas.use("def-" + origNode.type.toLowerCase()).attr({
      x: cx - 20,
      y: cy - 20,
      // id: origNode.data.id,
      class: "task-node"
      ...
      name = this.svgCanvas.text(cx, cy + 35, taskName).attr({
        dy: 3,
        class: "task-name"
      });
  ...
  private renderTaskLink(x1, y1, x2, y2, group) {
    let levelCentre = y1 + (y2 - y1) / 2,
      link = this.svgCanvas.path([
        ["M", x1, y1],
  ...
  private renderTaskRelation(cx, cy, origNode: Task, group) {
    // if (origNode.relation) {
      ...
      let relationText = group.select(".rel-text"),
        relationLink = null,
        textBox: BBox;
  ...
  private addDefs() {
    ...
    // Abstract
    let abstract = this.svgCanvas.el("g", {
      id: "def-abstract",
    });
    ...
    // Interaction
    let interaction = this.svgCanvas.el("g", {
      id: "def-interaction",
    });
    ...
    // System
    let system = this.svgCanvas.el("g", {
      id: "def-system",
    });
